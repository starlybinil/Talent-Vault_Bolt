/*
  ...
  1. Changes
    - Fix type casting for JSON fields
    - Handle null values properly
    - Ensure correct JSONB handling
*/
...
BEGIN
  ...
  IF profile_record.education IS NOT NULL THEN
    SELECT LEAST(
      jsonb_array_length(profile_record.education::jsonb),
      20  -- Cap at 20%
    ) INTO education_score;
  END IF;

  -- Experience (10%) - Check experience array length and required fields
  IF profile_record.experience IS NOT NULL THEN
    SELECT LEAST(
      jsonb_array_length(profile_record.experience::jsonb),
      10  -- Cap at 10%
    ) INTO experience_score;
  END IF;

  -- Projects (20%) - Check projects array length and required fields
  IF profile_record.projects IS NOT NULL THEN
    SELECT LEAST(
      jsonb_array_length(profile_record.projects::jsonb),
      20  -- Cap at 20%
    ) INTO projects_score;
  END IF;

  -- Skills (20%) - Check technical_skills and soft_skills
  IF profile_record.technical_skills IS NOT NULL THEN
    SELECT LEAST(
      jsonb_array_length(profile_record.technical_skills::jsonb),
      10  -- Technical skills contribute up to 10%
    ) INTO skills_score;
  END IF;

  IF profile_record.soft_skills IS NOT NULL THEN
    skills_score := skills_score + LEAST(
      jsonb_array_length(profile_record.soft_skills::jsonb),
